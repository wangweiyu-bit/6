...
(function(){
  ...
  async function guardPageAccess(){
    const path = location.pathname.replace(/\\+/g,'/');
    const isHome = path.endsWith('/pages/index.html') || path.endsWith('/index.html');
    const isCallback = path.endsWith('/pages/auth-callback.html') || path.endsWith('auth-callback.html');
    ...
    if(!m || !m.token){
      ...
      if(ok){
        ...
        location.href = (path.includes('/pages/') ? '../index.html' : 'index.html');
        return;
      }
    }
    // 若连接了后端，则进一步核验审批状态
    if(!apiState.mockMode){
      try{
        ...
        if(!me || !me.approved){ alert('需要管理员授权后才能访问'); location.href = (path.includes('/pages/') ? '../index.html' : 'index.html'); }
